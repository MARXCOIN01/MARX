...
uint256 hashGenesisBlock("0x00000d2cc6b7f42962160af9436ff7852e9e4fdb7659624ef1df5d1ccacad43c");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // MARX: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "MARX Signed Message:\n";
...
{
    // MARX: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 169000);
	
	if (nHeight > 169000*64)
		return nFees;
...
{
    // MARX: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
	...
	// Mainnet Genesis block:
		...
		const char* pszTimestamp = "marx genesis block";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1485290904;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2138862;
        ...
        assert(block.hashMerkleRoot == uint256("0x410ce9ab999a767bfb8b9eab369a21b5fd2b1653745b66bebb7ae87bc6437202"));
...
unsigned char pchMessageStart[4] = { 0xfe, 0xc3, 0xb9, 0xde }; // MARX: increase each by adding 6 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MARXMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MARXMiner : ProcessBlock, block not accepted");
    }
